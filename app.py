...
def main():
    ...
    if not creds:
        ...
        has_env = bool(os.environ.get("GOOGLE_REFRESH_TOKEN", ""))
        st.error("No API token found.")
        st.code(
            "Token file exists: {}\n"
            "GOOGLE_REFRESH_TOKEN env var set: {}\n"
            "Error: {}".format(
                token_path.exists(), has_env, get_last_error()
            )
        )
        st.stop()
    ...
    try:
        ...
        st.info("This usually means the Google token needs refreshing. Try reloading the page.")
